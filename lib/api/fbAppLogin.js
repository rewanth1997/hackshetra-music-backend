  ...
  if (err) {
   ...
   oauth2.get(musics, req.query.token, function(err, data ,response) {
    if(err) {
      ...
      res.redirect('/facebook/user_likes?token='+req.query.token);
    }
   })
...
router.get('/user_likes', function(req, res) {
  oauth2.get('https://graph.facebook.com/me/music', req.query.token, function(err, data ,response) {
  if (err) {
   console.error(err);
   res.send(err);
  }
